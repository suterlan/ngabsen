      ...
      <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow rounded-md sm:rounded-lg">
        <form method="POST" action="{{ route('admin.attendance.store') }}">
          ...
          <div class="mb-3">
              ...
              <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" :value="old('title')" required autofocus autocomplete="title" placeholder="Judul absensi" />
          ...
          <div class="mb-3">
            ...
            <textarea name="description" class="w-full h-20 mt-1 resize rounded-md text-slate-600 dark:text-slate-400 bg-white dark:bg-slate-900 text-sm border border-slate-300 outline-none focus:outline-none focus:ring-0 placeholder:text-slate-300 dark:placeholder:text-gray-500" placeholder="Maksimal 500 kata..." maxlength="500" required>{{old('description')}}</textarea>
          ...
          <div class="mb-3">
            ...
            <div class="flex-wrap h-32 overflow-auto border border-slate-300 rounded-md">
              <div class="grid grid-cols-2 sm:grid-cols-4 p-3 gap-3">
                @foreach ($jabatans as $jabatan_id => $name)
                  <label for="jabatan_id{{ $loop->index }}" class="inline-flex items-center">
                    <input id="jabatan_id{{ $loop->index }}" type="checkbox" class="rounded dark:bg-gray-900 border-gray-300 dark:border-gray-700 text-indigo-700 shadow-sm focus:ring-0 dark:focus:ring-indigo-700 dark:focus:ring-offset-gray-800 cursor-pointer" name="jabatan_ids[{{$jabatan_id}}]" value="{{$jabatan_id}}">
                    <span class="ms-1 me-3 text-sm text-gray-600 dark:text-gray-400">{{$name}}</span>
                  </label>
                @endforeach
              </div>
            </div>
          ...
          <div class="grid grid-cols-2 gap-5 mb-3 mt-1">
            ...
            <div>
              ...
              <x-text-input id="limit_start_time" class="block mt-1 w-full" type="time" name="limit_start_time" :value="old('limit_start_time')" required autofocus autocomplete="limit_start_time" />
          ...
          <div class="grid grid-cols-2 gap-5 mb-3 mt-1">
            ...
            <div>
              ...
              <x-text-input id="limit_end_time" class="block mt-1 w-full" type="time" name="limit_end_time" :value="old('limit_end_time')" required autofocus autocomplete="limit_end_time" />
          ...
          {{-- <div>
            ...
            <label for="code" class="inline-flex items-center">
              <input id="code" type="checkbox" class="rounded dark:bg-gray-900 border-gray-300 dark:border-gray-700 text-indigo-700 shadow-sm focus:ring-0 dark:focus:ring-indigo-700 dark:focus:ring-offset-gray-800 cursor-pointer" name="code">
