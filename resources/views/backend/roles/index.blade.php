<x-app-layout>
  ...
  <div class="pt-y">
                    ...
                    <x-table-td>
                      <div class="inline-block float-right md:hidden" x-data="{ aksi: false }">
                        ...
                        <div x-show="aksi" x-on:click.away="aksi = false" class="absolute z-50 right-5 mt-2 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 focus:outline-none" role="aksi" aria-orientation="vertical" aria-labelledby="aksi-button" tabindex="-1">
                            <div class="" role="none">
                                <a href="#" x-data="" x-on:click.prevent="$dispatch('open-modal', 'edit-role-{{$role->id}}')" class="text-gray-500 font-medium hover:text-gray-900 hover:bg-gray-50 block px-4 py-2 text-sm" role="aksiitem" tabindex="-1" id="aksi-item-0">
                                  Edit
                                </a>
                            </div>
                            <div class="" role="none">
                              <form x-data @submit.prevent="if(confirm('Yakin mau menghapus?\nData mungkin akan terhapus selamanya!')) $el.submit()" method="post" action="{{route('admin.roles.delete', $role->id)}}">
                                @csrf
                                @method('delete')
                      ...
                      <div class="items-center justify-end hidden md:flex space-x-2">
                        <x-edit-button x-data="" x-on:click.prevent="$dispatch('open-modal', 'edit-role-{{$role->id}}')" />
                        <form x-data @submit.prevent="if(confirm('Yakin mau menghapus?\nData mungkin akan terhapus selamanya!')) $el.submit()" method="post" action="{{route('admin.roles.delete', $role->id)}}">
                          @csrf
                          @method('delete')
                      ...
                      @include('backend.roles.partials.modal-edit')
    ...
    @include('backend.roles.partials.modal-create', $permissions)
