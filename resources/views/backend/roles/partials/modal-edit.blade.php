      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 p-3 gap-3">
        @foreach ($permissions as $permission)
          <label for="permission[{{$permission->id}}]" class="inline-flex items-center">
            <input id="permission[{{$permission->id}}]" type="checkbox" class="rounded dark:bg-gray-900 border-gray-300 dark:border-gray-700 text-indigo-700 shadow-sm focus:ring-0 dark:focus:ring-indigo-700 dark:focus:ring-offset-gray-800 cursor-pointer" name="permissions[]" value="{{$permission->name}}" @checked($role->hasPermissionTo($permission->name))>
