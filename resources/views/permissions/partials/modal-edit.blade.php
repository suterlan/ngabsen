<x-modal name="edit-permission-{{$permission->id}}" focusable>
  ...
  <form action="{{route('permission.update', $permission->id)}}" method="POST" class="p-6">
    @csrf
    @method('put')
  <div class="mt-6">
    <div>
      ...
      <div class="p-3 space-y-2">
        @foreach ($roles as $role)
          <label for="role[{{$role->id}}]" class="inline-flex items-center">
            <input id="role[{{$role->id}}]" type="checkbox" class="rounded dark:bg-gray-900 border-gray-300 dark:border-gray-700 text-indigo-700 shadow-sm focus:ring-0 dark:focus:ring-indigo-700 dark:focus:ring-offset-gray-800" name="roles[]" value="{{$role->name}}" @checked($role->hasPermissionTo($permission->name))>
